                ...
                <div class="tab-content">
                    <div id="home" class="container tab-pane active">
                                ...
                                <table class="table table-bordered table-striped table-vcenter js-dataTable-full dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                    ...
                                    <tbody>
                                        @foreach ($active_service_enquiries as $active_service_enquiry )
                                        <tr role="row" class="odd">
                                        <td class="text-center font-size-sm sorting_1">{{ $loop->index+1 }}</td>
                                        <td class="font-w600 font-size-sm">
                                            {{ $active_service_enquiry->name }}
                                        </td>

                                      

                                        <td class="font-w600 font-size-sm">
                                            {{ $active_service_enquiry->email }}
                                        </td>

                                        <td class="font-w600 font-size-sm">
                                            {{ $active_service_enquiry->mobile }}
                                        </td>

                                        <td class="font-w600 font-size-sm">
                                            {{ $active_service_enquiry->subject }}
                                        </td>

                                    @if ($active_service_enquiry->status == 1)
                                        <td class="d-none d-sm-table-cell"><a href="{{ route('service.enquiries.status', [$active_service_enquiry->id]) }}"
                                    ...
                                    <td>
                                    <div class="btn-group">
                                        <a href="{{ route('service_enquiries.show',[$active_service_enquiry->id]) }}" method="GET" >     
                                        ...
                                        <form action="{{ route('service_enquiries.destroy',$active_service_enquiry->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div id="menu1" class="container tab-pane fade"><br>
                        <table class="table table-bordered table-striped table-vcenter js-dataTable-full dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                                ...
                                <tbody>
                                        @foreach ($deactive_service_enquiries as $deactive_service_enquiry )
                                                <tr role="row" class="odd">
                                                <td class="text-center font-size-sm sorting_1">{{ $loop->index+1 }}</td>
                                                <td class="font-w600 font-size-sm">
                                                    {{ $deactive_service_enquiry->name }}
                                                </td>

                                                <td class="font-w600 font-size-sm">
                                                    {{ $deactive_service_enquiry->email }}
                                                </td>

                                                <td class="font-w600 font-size-sm">
                                                    {{ $deactive_service_enquiry->mobile }}
                                                </td>

                                                <td class="font-w600 font-size-sm">
                                                    {{ $deactive_service_enquiry->subject }}
                                                </td>

                                            @if ($deactive_service_enquiry->status == 1)
                                                <td class="d-none d-sm-table-cell"><a href="{{ route('service.enquiries.status', [$deactive_service_enquiry->id]) }}"
                                                        class="btn btn-success btn-sm">Not Read</a></td>
                                            @else
                                            ...
                                            <td>
                                            <div class="btn-group">
                                                <a href="{{ route('service_enquiries.show',[$deactive_service_enquiry->id]) }}" method="GET" >     
                                                    <button  type="submit" class="btn btn btn-sm " >
                                                            <i class="far fa-0x fa-eye"></i>
                                                        </button>
                                                </a> 

                                                <form action="{{ route('service_enquiries.destroy',$deactive_service_enquiry->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
