        ...
        <div class="col-lg-6">
            <!-- Block Tabs Default Style -->
                ...
                <div class="tab-content">
                    ...
                    <tbody>
                        @foreach ($active_membership_enquiries as $active_membership_enquiry )
                        <tr role="row" class="odd">
                        <td class="text-center font-size-sm sorting_1">{{ $loop->index+1 }}</td>
                        <td class="font-w600 font-size-sm">
                            {{ $active_membership_enquiry->name }}
                        </td>

                        <td class="font-w600 font-size-sm">
                            {{ $active_membership_enquiry->email }}
                        </td>

                        <td class="font-w600 font-size-sm">
                            {{ $active_membership_enquiry->mobile }}
                        </td>
                       

                        @if ($active_membership_enquiry->status == 1)
                          <td class="d-none d-sm-table-cell"><a href="{{ route('membership_enquiries.status', [$active_membership_enquiry->id]) }}"
                      ...
                      <td>
                      <div class="btn-group">
                        <a href="{{ route('membership_enquiries.show',[$active_membership_enquiry->id]) }}" method="GET" >     
                     ...
                     <form action="{{ route('membership_enquiries.destroy',$active_membership_enquiry->id) }}" method="POST">
                       @csrf
                       @method('DELETE')
                    ...
                    <div id="menu1" class="container tab-pane fade"><br>
                        ...
                        @if ($deactive_membership_enquiry->status == 1)
                          <td class="d-none d-sm-table-cell"><a href="{{ route('membership_enquiries.status', [$deactive_membership_enquiry->id]) }}"
                                  class="btn btn-success btn-sm">Not Read</a></td>
                      @else
                      ...
                      @endif
                        ...
                        <td>
                      <div class="btn-group">
                        <a href="{{ route('membership_enquiries.show',[$deactive_membership_enquiry->id]) }}" method="GET" >     
                              <button  type="submit" class="btn btn btn-sm " >
                                       <i class="far fa-0x fa-eye"></i>
                                </button>
                        </a> 
                        ...
                        <form action="{{ route('membership_enquiries.destroy',$deactive_membership_enquiry->id) }}" method="POST">
                       @csrf
                       @method('DELETE')
